  ...
  return `
    <div class="item">
      ...
      <div class="item-actions">
        <strong>${money(x.amount)}</strong>
        <button class="delete" onclick="removeItem('${type}','${x.id}')">×</button>
      </div>
    </div>
  `;
}
...
document
  ...
  .addEventListener("click", () => {
    ...
    if (!amount) {
      result.textContent = "Enter an amount to check.";
...
document
  ...
  .addEventListener("click", () => {
    ...
    if (affordResult) {
      ...
      affordResult.className = "result muted";
    }
